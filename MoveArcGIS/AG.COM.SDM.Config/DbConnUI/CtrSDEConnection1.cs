...
namespace AG.COM.SDM.Config
{
    ...
    /// SDE连接用户控件类
    /// </summary>
    public partial class CtrSDEConnection1 : UserControl
    {
        /// <summary>
        /// 实例化新对象
        /// </summary>
        public CtrSDEConnection1()
        {
            //初始化界面组件
            InitializeComponent();
        ...
        private IPropertySet GetPropertySet()
        {
            ...
            tPropertySet.SetProperty("DataBaseType",cmbDataType.Text);
            //SDE服务器名称
        ...
        private void cmbDataType_SelectedValueChanged(object sender, EventArgs e)
        {
            if(cmbDataType.SelectedItem.ToString()=="Oracle")
            {
                ...
                {
                    txtInstance.Text = "sde:oracle11g:localhost/orcl";
                }else
                {
                    txtInstance.Text =$"sde:oracle11g:{txtServerName.Text}/orcl";
                }
            ...
            else
            {
                if (string.IsNullOrWhiteSpace(txtServerName.Text))
                {
                    txtInstance.Text = "sde:postgresql:localhost";
                }
                else
                {
                    txtInstance.Text = $"sde:postgresql:{txtServerName.Text}";
                }
