...
namespace NetTopologySuite.Features.Fields
{
    ...
    {
        public static readonly int MaxFieldLength = 19;    // Decimal point and any sign is included in field width, if present. 
        public static readonly int MaxFieldPrecision = 17; // dBASE specs states it could be max 15, but Esri uses up to 17.

        public DbfNumericField(string name, DbfType type, int length, int precision)
            : base(name, type, length, precision)
        {
            // -0.12345 => legth: 8, decimalCount: 5 => should be (length - 3). But Esri allows decimalCount: 6 => (length - 2).
        }

        public static DbfField Create(string name, int length, int precision)
        {
    ...
    public abstract class DbfNumericField<T> : DbfNumericField where T : struct, IConvertible, IFormattable
    {
        ...
        public override void WriteValue(Stream stream)
        {
            ...
            if (valueText.Length < Length)
            {
                valueText = valueText.PadLeft(Length, ' '); // PadLeft for Values
            }
