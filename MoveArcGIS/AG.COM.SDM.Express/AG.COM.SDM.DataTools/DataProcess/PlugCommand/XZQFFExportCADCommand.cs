...
{
    public class XZQFFExportCADCommand : BaseTool, IUseIcon
    {
        private IHookHelperEx m_hookHelper = new HookHelperEx();

        private FormXZQFFExportCAD m_FormXZQFFExportCAD = null;

        /// <summary>
        /// 初始化对象实例
        /// </summary>
        public XZQFFExportCADCommand()
        {
        ...
        public override void OnClick()
        {
            try
            {
                ...
                if (m_FormXZQFFExportCAD == null)
                {
                    m_FormXZQFFExportCAD = new FormXZQFFExportCAD(tInitTag, m_hookHelper);
                    m_FormXZQFFExportCAD.FormClosed += new FormClosedEventHandler(FormSelectExtentPrint_OnFormClosed);
                    m_FormXZQFFExportCAD.ExtentFeatureClassName = XZQFeatureClass;
                    m_FormXZQFFExportCAD.ExtentFieldName = XZQField;
                    m_FormXZQFFExportCAD.TFFeatureClassName = TFFeatureClass;
                    m_FormXZQFFExportCAD.TFFieldName = TFField;
                    m_FormXZQFFExportCAD.Show(m_hookHelper.Win32Window);
                }
                else
                {
                    if (!m_FormXZQFFExportCAD.Visible)
                    {
                        m_FormXZQFFExportCAD.Show(m_hookHelper.Win32Window);
                    }
                    m_FormXZQFFExportCAD.WindowState = FormWindowState.Normal;
                }
            }
            catch (Exception ex)
            {
        ...
        {
            if (m_FormXZQFFExportCAD == null) return;
            ...
            if (m_FormXZQFFExportCAD != null && m_FormXZQFFExportCAD.WindowState == FormWindowState.Minimized)
            {
                m_FormXZQFFExportCAD.WindowState = FormWindowState.Normal;

                if (tGeometry != null)
                {
                    m_FormXZQFFExportCAD.OuterSelectExtent(tGeometry);
                    m_hookHelper.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewForeground, null, null);
                }
            }
        }
