...
{
    public partial class FormSelectExtentExportCAD : Form
    {
        #region 变量
        ...
        public FormSelectExtentExportCAD(SelectExtentExportCADInitTag tInitTag, IHookHelperEx tHookHelper)
        {
            InitializeComponent();
            ...
            try
            {
                ...
                foreach (object obj in lstAllExtent.SelectedItems)
                {
                    ...
                    while (tFeature != null)
                    {
                        ...
                        //文件名不带扩展名
                        tFenFu.FileName = txtOutput.Text + "\\" + value + "_" + Convert.ToString(tFeature.get_Value(m_ExtentFieldIdx));
                ...
                //导出CAD
                VectorClip2CADMultiHelper tVectorClip2CADMultiHelper = new VectorClip2CADMultiHelper(tLayers,
                cmbCADTemplate.Text, m_hookHelper.ActiveView, chkExportElement.Checked, cmbCADVersion.Text, tFenFus);
                if (tVectorClip2CADMultiHelper.MainProcess() == true)
                {
                    Close();
                }
            }
