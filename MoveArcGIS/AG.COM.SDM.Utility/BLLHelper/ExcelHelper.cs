    ...
    public class ExcelHelper
    {
        ...
        public static DataTable GetDataTableFromExcel(string fileName, string sheetName)
        {
            ...
            for (int i = 0; i < sheetNames.Rows.Count; i++)
            {
                ...
                if (name.EqualIgnoreCase(sheetName))
                {
                    idxSheet = i;
                    break;
                }
        ...
        /// <param name="fileName"></param>
        /// <returns></returns>
        public static List<string> GetAllSheets(string fileName)
        {
            Dictionary<string, DataTable> tReturn = new Dictionary<string, DataTable>();
            var fileType = System.IO.Path.GetExtension(fileName.ToLower()).Trim();
            var excelVersionNumber = fileType == ".xlsx" ? "12.0" : "8.0";
            //源的定义
            //string strConn = "Provider=Microsoft.ACE.OLEDB.12.0;" + "Data Source=" + strExcelFileName + ";" + "Extended Properties='Excel " + excelVersionNumber + ";HDR=YES;IMEX=1';";
            var strConn = "";
            //if (fileType == ".xls")
            //    strConn = string.Format("Provider=Microsoft.Jet.OLEDB.4.0; Data Source={0} ;Extended Properties=\"Excel {1};HDR=Yes;IMEX=2;\"", fileName, excelVersionNumber);
            //else
            strConn = "Provider=Microsoft.ACE.OLEDB.12.0;" + "Data Source='" + fileName + "';" + "Extended Properties='Excel " + excelVersionNumber + ";HDR=Yes;IMEX=1';";
            ...
            foreach (DataRow dr in sheetNames.Rows)
            {
                string sheet = Convert.ToString(dr[2]);

                //以下名称的sheet其实不存在
                if (sheet == "ExternalData_1") continue;
